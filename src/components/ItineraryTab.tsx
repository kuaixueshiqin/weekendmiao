...
import { MapPin, Utensils, Hotel, CheckCircle2, Clock, AlertCircle, ChevronDown, Plus, Map as MapIcon, List, Trash2, RefreshCw, X, Heart, ShoppingCart, CircleDot, Loader2 } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useTrips, type TripItem, type Status } from "@/hooks/useTrips";
...
const borderColors = { scenic: "#3b9eff", food: "#ff7a25", hotel: "#9c6dfa" };

// Replacement suggestions for "换一个"
...
const ItineraryTab = () => {
  ...
  const handleAddTrip = async () => {
    ...
    setCreating(true);
    const trip = await createTrip(newTitle, newDates);
    setCreating(false);
    setNewTitle("");
    setNewDates("");
    setShowAddTrip(false);
    if (trip) {
      // Make newly created trip active
      setActiveTrip(trip.id);
    }
  };

  const handleRefreshItem = async (item: TripItem) => {
    const repl = replacements[item.type] || replacements.scenic;
    await replaceItem(item.id, repl);
  };

  const handleBookAll = async () => {
    if (activeTrip) await bookAll(activeTrip.id);
  };

  const unbookedCount =
    activeTrip?.days.reduce((sum, d) => sum + d.items.filter((i) => i.status === "unbooked").length, 0) || 0;

  const completedCount =
    activeTrip?.days.reduce((sum, d) => sum + d.items.filter((i) => i.status === "completed").length, 0) || 0;
  ...
  return (
    <div className="bg-background min-h-full">
      {/* ── Sticky header ── */}
      <div className="sticky top-0 z-20 px-4 pt-4 pb-3 bg-background">
        {/* Title */}
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {activeTrip
                ? `${activeTrip.dates} · ${activeTrip.days.length} 个行程段`
                : "暂无行程，点击 + 新建"}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="w-9 h-9 rounded-2xl bg-muted border border-border/60 flex items-center justify-center hover:bg-secondary transition-colors"
            >
        ...
        {/* View Toggle */}
        <div
          className="flex bg-card rounded-2xl p-1 border border-border/50"
          style={{ boxShadow: "var(--shadow-card)" }}
        >
          <button
      ...
      <div className="px-4 pb-6 pt-3 space-y-3">
        ...
        {activeTrip && unbookedCount > 0 && (
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl font-bold text-sm text-primary-foreground transition-all active:scale-[0.98] bg-primary"
          >
        ...
        {/* Trip switcher (if multiple trips) */}
        {trips.length > 1 && (
        ...
        {!activeTrip && (
          <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
            <div className="w-16 h-16 rounded-3xl bg-muted flex items-center justify-center text-3xl">🗺️</div>
            <p className="font-semibold text-sm">还没有行程</p>
            <p className="text-xs text-muted-foreground max-w-[200px]">
              点击右上角 <span className="font-bold text-foreground">+</span> 新建行程，<br />
              或在 AI 对话页让小喵帮你规划
            </p>
            <button
              onClick={() => setShowAddTrip(true)}
              className="mt-2 px-5 py-2.5 rounded-2xl bg-primary text-primary-foreground text-sm font-bold transition-all active:scale-[0.98]"
            >
              新建行程
            </button>
          </div>
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 bg-primary">
                        <span className="text-primary-foreground font-extrabold text-sm">D{day.day}</span>
                      </div>
                    ...
                    <div
                      className={`w-7 h-7 rounded-xl bg-muted flex items-center justify-center transition-transform ${
                              ...
                              >
                                <div className="flex items-center gap-2.5">
                                  ...
                                  <div className="flex flex-col items-end gap-1 shrink-0">
                                    ...
                                    <span className="text-xs font-bold" style={{ color: "hsl(var(--meituan-red))" }}>
                                      {item.price}
                                    </span>
                                  </div>
                                </div>
          ...
          >
            <div className="aspect-[4/3] bg-[hsl(210_20%_95%)] relative overflow-hidden">
              ...
              <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 300">
                <polyline
                  points="120,60 200,100 300,80 280,170 160,200 240,250"
                  fill="none"
              ...
              <div className="absolute top-[18%] left-[28%] flex flex-col items-center">
                <div className="w-7 h-7 rounded-full bg-meituan-blue flex items-center justify-center shadow-md border-2 border-card">
              ...
              <div className="absolute top-[55%] left-[65%] flex flex-col items-center">
                <div className="w-7 h-7 rounded-full bg-meituan-orange flex items-center justify-center shadow-md border-2 border-card">
              ...
              <div className="absolute top-[78%] left-[55%] flex flex-col items-center">
                <div className="w-7 h-7 rounded-full bg-purple-500 flex items-center justify-center shadow-md border-2 border-card">
            ...
            <div className="flex items-center justify-center gap-4 py-2 border-t border-border">
              <div className="flex items-center gap-1">
                <div className="w-3 h-3 rounded-full bg-meituan-blue" />
                <span className="text-[10px] text-muted-foreground">景点</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="w-3 h-3 rounded-full bg-meituan-orange" />
                <span className="text-[10px] text-muted-foreground">美食</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="w-3 h-3 rounded-full bg-purple-500" />
                <span className="text-[10px] text-muted-foreground">酒店</span>
              </div>
            </div>
          </div>
        ) : null}
                ...
                <div className="px-5 overflow-y-auto min-h-0 flex-1">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {selectedItem.time} · {selectedItem.description}
                      </p>
                    </div>
                  ...
                  <div className="bg-muted/60 rounded-2xl p-4 mb-4 border border-border/50">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-extrabold text-lg" style={{ color: "hsl(var(--meituan-red))" }}>
                        {selectedItem.price}
                      </span>
                    </div>
                    {selectedItem.code && (
                ...
                <div className="px-5 pb-6 pt-2 shrink-0 bg-card">
                  <div className="flex gap-2">
                    {selectedItem.status === "unbooked" && (
                      <button
                        onClick={() => {
                          const code = `MT${Date.now().toString().slice(-8)}-${Math.floor(Math.random() * 9000 + 1000)}`;
                          updateItemStatus(selectedItem.id, "pending", code);
                          setSelectedItem({ ...selectedItem, status: "pending", code });
                        }}
                        className="flex-1 py-3 rounded-2xl font-bold text-sm text-primary-foreground transition-all active:scale-[0.98] bg-primary"
                      >
                        立即预定
                      </button>
                    )}
                    {selectedItem.status === "pending" && (
                      <>
                        <button className="flex-1 py-3 rounded-2xl font-bold text-sm text-primary-foreground transition-all bg-primary">
                          {selectedItem.code ? "出示凭证" : "立即购买"}
                        </button>
                        <button className="px-5 py-3 bg-muted rounded-2xl font-bold text-sm hover:bg-secondary transition-colors">
                          导航
                        </button>
                      </>
                    )}
                    ...
                    {selectedItem.status === "completed" && (
                      <div className="flex-1 py-3 rounded-2xl font-bold text-sm text-center text-meituan-green bg-meituan-green/10">
                        ✓ 核销成功
                      </div>
                    )}
                  </div>
                    ...
                    favorites.map((trip) => (
                      <div key={trip.id} className="p-4 rounded-2xl bg-muted/50 border border-border/50">
                        ...
                        <div className="flex gap-2 mt-3">
                          <button
                            onClick={() => { setActiveTrip(trip.id); setShowFavorites(false); }}
                            className="flex-1 py-2 rounded-xl text-xs font-bold text-primary-foreground transition-all bg-primary"
                          >
                            查看详情
                          </button>
                          <button
              ...
              >
                ...
                <div className="p-5 space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-3.5 rounded-2xl font-bold text-sm text-primary-foreground transition-all disabled:opacity-40 active:scale-[0.98] bg-primary flex items-center justify-center gap-2"
                  >
